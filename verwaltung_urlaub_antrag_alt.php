<?php
###### Editnotes ####
#$LastChangedDate: 2011-09-19 11:49:47 +0200 (Mo, 19 Sep 2011) $
#$Author: msternberg $ 
#####################
...
if (!isset($_POST['antrag']))
 ...
 if (isset($_REQUEST['xShort'])) // zeige Ausschnitt
    ...
    while ($zeile_level=mysql_fetch_array($ergebnis_level))
        ...
        $stil = '<td>';
             ...
             if ($kalender[$zeile_level['ule_id']][$zeile_ma['hma_id']][$test]['Bereitschaft'] == 1)
                ...
                {
                $bild_bereit='<img src="bilder/icon_action.gif" alt="Backup" title="Backup">';
                }
            else
                {
                $bild_bereit=Null;
                ...
                echo $stil . $bild . $bild_release . $bild_bereit . '</td>';
                } // For Tage Schleife
    ...
    $sql_filter='SELECT hma_id, hma_login FROM mitarbeiter WHERE hma_id > 2 AND hma_aktiv = 1 ' .
        'ORDER BY hma_login';
    ...
    echo '<td><input type="radio" name="hka_dienst" value="1">&nbsp;OnCall&nbsp;</td>';

    echo '<td><input type="radio" name="hka_dienst" value="2">&nbsp;OnCall (Backup)</td>';  
    ...
    foreach ($datumselement AS $Kalendertag)
        ...
        WHERE hka_hmaid = "' . $Daten['hka_hmaid'] . '" AND hka_tag = "' . $Kalendertag . '"';
            ...
            $sql_log='INSERT INTO eventlog (' .
                ...
                '"Vacation", ' .
                '"' . $_SESSION['hma_login'] . '" ,' .
                '"has changed the vacations for ' . $Daten['hka_hmaid'] . '")';
        ...
        $sql_log='INSERT INTO eventlog (' .
            ...
            '"Vacation", ' .
            '"' . $_SESSION['hma_login'] . '" ,' .
            '"has entered new vacations for ' . $Daten['hka_hmaid'] . '")';
