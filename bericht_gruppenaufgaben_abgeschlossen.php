...
echo '<li id="current"><a href="#">abgeschlossene Gruppenaufgaben letzte Woche ('.$anzahl.')</a></li>';
...
echo '<li><a href="#">abgeschlossene Gruppenaufgaben letzte Woche ('.$anzahl.')</a></li>';
...
$sql='SELECT *, m1.hma_login AS inhaber, m2.hma_login AS mitarbeiter FROM aufgaben 
    ...
    WHERE hau_abschluss = 1 AND (hau_abschlussdatum BETWEEN "'.date('Y-m-d',mktime(0,0,0,date('n'),date('j')-7,date('Y'))).'" 
      AND "'.date('Y-m-d',mktime(0,0,0,date('n'),date('j'),date('Y'))).'") AND hau_aktiv = 1 ' . 
            $_SESSION['filterstring'].' 
    GROUP BY hau_id 
