<?php
###### Editnotes ####
#$LastChangedDate: 2012-02-03 11:53:00 +0100 (Fr, 03 Feb 2012) $
#$Author: msternberg $ 
#####################
...
foreach ($anzeigefelder as $bezeichner => $inhalt)
    ...
    switch ($bezeichner)
        ...
        case 'Mitarbeiter':
            ...
            $sql_filter='SELECT hma_id, hma_login FROM mitarbeiter WHERE hma_id > 3 ' .
                'ORDER BY hma_login';
    ...
    #$zeile[$inhalt] = htmlspecialchars($zeile[$inhalt],ENT_QUOTES);
        ...
        switch ($bezeichner)
            ...
            case 'Ticket':
              ...
              case 'aktualisiert':
              /*
                              $sql_update =  'SELECT ulo_zeitstempel AS letzte_bearbeitung FROM log
                                WHERE ulo_aufgabe = '.$zeile['hau_id'].'
                                ORDER BY ulo_zeitstempel DESC
                                LIMIT 1';
                ...
                if (!$ergebnis_update=mysql_query($sql_update, $verbindung))
                    ...
                    fehler();
                     ...
                     echo '<td bgcolor="' . $color . '" class="' . $font . '">' . zeitstempel_anzeigen($zeile['ulo_zeitstempel']) . '</td>'; 
                   break;
