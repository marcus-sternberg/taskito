...
echo '<li><a href="bericht_gruppenaufgaben_offen.php">Gruppenaufgaben ohne Bearbeiter ('.$anzahl_ohne_bearbeiter.')</a></li>';
...
while ($zeile=mysql_fetch_array($ergebnis))
{
$anzahl_zugewiesen_wartet=$zeile['anzahl'];
}
...
while ($zeile=mysql_fetch_array($ergebnis))
{
$anzahl_zugewiesen_arbeit=$zeile['anzahl'];
$anzahl_gesamt=$anzahl_zugewiesen_arbeit+$anzahl_zugewiesen_wartet;
}
    

echo '<li id="current"><a href="#">Gruppenaufgaben in Bearbeitung ('.$anzahl_gesamt.')</a></li>';
...
echo '<li><a href="#">Gruppenaufgaben in Warteschlange ('.$anzahl_zugewiesen_wartet.')</a></li>';
...
$sql='SELECT *, m1.hma_login AS inhaber, m2.hma_login AS mitarbeiter FROM aufgaben 
    ...
    LEFT JOIN aufgaben_zuordnung ON uaz_pg = m2.hma_level
    LEFT JOIN level ON uaz_pg = ule_id 
...
echo '<li><a href="#">Gruppenaufgaben in Bearbeitung ('.$anzahl_zugewiesen_arbeit.')</a></li>';
...
$sql='SELECT *, m1.hma_login AS inhaber, m2.hma_login AS mitarbeiter FROM aufgaben 
    ...
    LEFT JOIN projekte ON hau_hprid = hpr_id   
    INNER JOIN prioritaet ON hau_prio = upr_nummer  
    WHERE uau_status = 0 AND uau_ma_status =1 AND hau_aktiv = 1 ' . $_SESSION['filterstring'] .
    'GROUP BY hau_id ' .
    'ORDER BY hau_prio DESC, hau_pende'; 
